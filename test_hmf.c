...
/*	double vals[] =
	...
	double *y = (double*)malloc(mesh*n_species*sizeof(double));

	double h = 1e-8;
	double aerr;
	double *dfdy = (double*)malloc(mesh*n_species*mesh*n_species*sizeof(double));
	double *dydta = (double*)malloc(mesh*n_species*mesh*n_species*sizeof(double));
	double *dydtb = (double*)malloc(mesh*n_species*mesh*n_species*sizeof(double));
	double *dfdk = (double*)malloc(mesh*n_species*mesh*n_react*sizeof(double));
	...
	double *dfdlogk0 = (double*)malloc(mesh*n_species*n_react*sizeof(double));
	double *dfddlogk = (double*)malloc(mesh*n_species*n_react*sizeof(double));
...
/*	// Random Rate Constants
	...
	hmca_logexp_set(logk0, dlogk, rates, weights, n_unimol, n_bimol, mesh, bound);
	...
	for (i = 0; i < mesh; ++i)
	{
		for (j = 0; j < n_species; ++j)
			printf(" %+.2f", y[n_species*i+j]);
		printf("\n");
	}


	// Function and Jacobian
	hmca_hmf_func(y, dydta, n_species_0, n_species_1, n_unimol, n_bimol, mesh, reactions, rates, weights, hmca_hmf_nn_2x1);
	hmca_hmf_jac(y, dfdy, n_species_0, n_species_1, n_unimol, n_bimol, mesh, reactions, rates, weights, hmca_hmf_nn_2x1);

	printf("f =\n");
	for (i = 0; i < mesh; ++i)
	{
		for (j = 0; j < n_species; ++j)
			printf(" %+.6f", dydta[n_species*i+j]);
		printf("\n");
	}

	printf("J =\n");
	for (i = 0; i < mesh*n_species; ++i)
	{
		for(j = 0; j < mesh*n_species; ++j)
			printf(" %+.6f", dfdy[mesh*n_species*i+j]);
		printf("\n");
	}

	// Compare to Numerica Jacobian
	printf("J_n - J =\n");
	for (i = 0; i < mesh*n_species; ++i)
	{
		if (y[i] > 0.0)
		{
			...
			hmca_hmf_func(y, dydta, n_species_0, n_species_1, n_unimol, n_bimol, mesh, reactions, rates, weights, hmca_hmf_nn_2x1);
			y[i] -= 2.0*h;
			hmca_hmf_func(y, dydtb, n_species_0, n_species_1, n_unimol, n_bimol, mesh, reactions, rates, weights, hmca_hmf_nn_2x1);
			y[i] += h;
			for(j = 0; j < mesh*n_species; ++j)
			{
				aerr = (dydta[j]-dydtb[j])/(2.0*h) - dfdy[mesh*n_species*j+i];
				printf(" %+.6f", aerr);
			}
		}
		else
		{
			...
			for(j = 0; j < mesh*n_species; ++j)
			{
				aerr = (dydta[j]-dydtb[j])/h - dfdy[mesh*n_species*j+i];
				printf(" %+.6f", aerr);
			}
	...
	hmca_hmf_dfdk(y, dfdk, n_species_0, n_species_1, n_unimol, n_bimol, mesh, reactions, rates, weights, hmca_hmf_nn_2x1);
	...
	hmca_logexp_set_dkdx(logk0, dlogk, dkdlogk0, dkddlogk, n_unimol, n_bimol, mesh, bound);
	...
	{
		logk0[i] += h;
		hmca_logexp_set(logk0, dlogk, ka, wt, n_unimol, n_bimol, mesh, bound);
		logk0[i] -= 2.0*h;
		hmca_logexp_set(logk0, dlogk, kb, wt, n_unimol, n_bimol, mesh, bound);
		logk0[i] += h;
	...
	memset(dfdlogk0, 0, mesh*n_species*n_react*sizeof(double));
	for (i = 0; i < mesh*n_species; ++i) for (j = 0; j < mesh*n_react; ++j) for (k = 0; k < n_react; ++k)
		dfdlogk0[n_react*i+k] += (j%n_react == k%n_react) * dfdk[mesh*n_react*i+j] * dkdlogk0[j];

	memset(dfddlogk, 0, mesh*n_species*n_react*sizeof(double));
	for (i = 0; i < mesh*n_species; ++i) for (j = 0; j < mesh*n_react; ++j) for (k = 0; k < n_react; ++k)
	...
	for (i = 0; i < n_react; ++i)
	{
		logk0[i] += h;
		hmca_logexp_set(logk0, dlogk, ka, wt, n_unimol, n_bimol, mesh, bound);
		hmca_hmf_func(y, dydta, n_species_0, n_species_1, n_unimol, n_bimol, mesh, reactions, ka, weights, hmca_hmf_nn_2x1);
		logk0[i] -= 2.0*h;
		hmca_logexp_set(logk0, dlogk, kb, wt, n_unimol, n_bimol, mesh, bound);
		hmca_hmf_func(y, dydtb, n_species_0, n_species_1, n_unimol, n_bimol, mesh, reactions, kb, weights, hmca_hmf_nn_2x1);
		logk0[i] += h;
		for(j = 0; j < mesh*n_species; ++j)
		{
